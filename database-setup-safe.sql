/*
 * Rob-AI - Safe Database Setup (handles existing objects)
...
DROP POLICY IF EXISTS "Public read access to templates" ON content_templates;
...
-- RLS Policies (recreated after dropping)
...
-- Drop existing function and trigger to recreate
DROP TRIGGER IF EXISTS trigger_update_user_analytics ON conversations;
...
-- Drop existing cleanup function to recreate
...
-- Clear existing data to avoid conflicts, then insert fresh templates
...
UNION ALL
SELECT 'user_analytics' as table_name, count(*) as row_count FROM user_analytics
UNION ALL
SELECT 'content_templates' as table_name, count(*) as row_count FROM content_templates
UNION ALL
