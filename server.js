...
const requiredEnvVars = ['OPENAI_API_KEY', 'SUPABASE_URL', 'SUPABASE_ANON_KEY', 'ENCRYPTION_KEY', 'TAVILY_API_KEY', 'JWT_SECRET'];
...
const authLimiter = rateLimit({
  ...
  message: {
    ...
    error_es: 'Demasiados intentos de autenticación, intenta de nuevo más tarde'
  },
...
const generalLimiter = rateLimit({
  ...
  message: {
    ...
    error_es: 'Demasiadas solicitudes, intenta de nuevo más tarde'
  },
  standardHeaders: true,
  legacyHeaders: false
});
...
const authenticateToken = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).json({
      ...
      error_es: 'Token de acceso requerido'
    });
  }
  ...
  try {
    ...
    if (error.name === 'TokenExpiredError') {
      ...
      return res.status(500).json({
        ...
        error_es: 'Error de autenticación'
      });
    }
  }
};
...
app.post('/api/chat-with-voice', authenticateToken, async (req, res) => {
  try {
